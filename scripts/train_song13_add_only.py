...
from font_machine_learn.song13_add_only import export_song13_add_only  # noqa: E402


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run Stage24 source-preserving Song13 add-only adapter.")
    ...
    parser.add_argument("--out-dir", type=Path, default=STAGE24_SONG13_ADD_ONLY)
    parser.add_argument("--metadata", type=Path, default=SONG13_ADD_ONLY_METADATA)
    parser.add_argument("--contact-sheet", type=Path, default=SONG13_ADD_ONLY_CONTACT)
    parser.add_argument("--error-contact-sheet", type=Path, default=SONG13_ADD_ONLY_ERROR_CONTACT)
    parser.add_argument("--threshold", type=float, action="append", default=[])
    parser.add_argument("--add-patch-radius", type=int, default=3)
    parser.add_argument("--style-patch-radius", type=int, default=4)
    parser.add_argument("--max-train-glyphs", type=int, default=None)
    parser.add_argument("--add-hidden-units", type=int, default=48)
