...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the CJK-focused edge-transition refinement.")
    parser.add_argument("--source-metadata", type=Path, default=SOURCE_METADATA)
    parser.add_argument("--out-dir", type=Path, default=CJK_EDGES_DIR)
    parser.add_argument("--metadata", type=Path, default=CJK_EDGES_METADATA)
    parser.add_argument("--search-json", type=Path, default=CJK_EDGES_SEARCH)
    parser.add_argument("--contact-sheet", type=Path, default=CJK_EDGES_CONTACT)
    parser.add_argument("--worst-contact-sheet", type=Path, default=CJK_EDGES_WORST_CONTACT)
